...
class TestPaymentRequestForm(Form):
    sid = sid_field
    token = StringField('Secret Token', [validators.DataRequired(), validators.Length(max=64)])
    amount = DecimalField('Amount', [validators.DataRequired()], places=2)
    pid = pid_field
    service = StringField('Service URL', [validators.DataRequired(), validators.URL(require_tld=False)])
...
class TestPaymentResponseForm(Form):
    pid = pid_field
    ref = StringField('Reference ID', [validators.DataRequired(), validators.Length(max=64)])
