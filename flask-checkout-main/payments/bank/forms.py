...
pid_field = StringField('Payment ID', [validators.DataRequired(), validators.Length(max=64)])
checksum_field = StringField('Checksum', [validators.DataRequired()])


class PaymentRequestForm(Form):
    sid = sid_field
    pid = pid_field
    amount = DecimalField('Amount', [validators.DataRequired(), validators.NumberRange(min=0)], places=2)
    checksum = checksum_field
    success_url = StringField('Success URL', [validators.DataRequired(), validators.URL(require_tld=False)])
    cancel_url = StringField('Cancel URL', [validators.DataRequired(), validators.URL(require_tld=False)])
    error_url = StringField('Error URL', [validators.DataRequired(), validators.URL(require_tld=False)])
